...
trap '
  sshpass -p Password1 ssh -p 22 -o ConnectTimeout=10 -o StrictHostKeyChecking=no root@${APP}.${DISTRO}.com "journalctl -u snap.${APP}.webui --no-pager | tail -500" > "$ART/journalctl.${APP}.log" 2>&1 || true
